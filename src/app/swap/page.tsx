...
export default function SwapPage() {
  return (
    <PageBody>
      <PageHeader
        title="調堂安排"
        description="老師病假／事假／公假要調堂：可即時揀建議再轉入紀錄。公假不計算代堂 ±。已轉入代堂紀錄嘅同事該節會佔用，之後再產生調堂唔會派同一人同一節去另一班。CLP 可以調堂。同一科可兩堂一拼調，但調堂後同一班同一日同一科不能有 4 堂或以上（例如 4A 不能一日上四堂中文）。普通話／戲劇若另一位老師在，由該老師合班，不計節數。調堂要課室無人用。可複合多名老師。重摘課不能調堂。"
      />
      <ScheduleGate>
        <SwapInner />
      </ScheduleGate>
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            1）揀請假種類同日期　2）即時揀建議轉入紀錄　3）CLP 可以調堂　4）同一科兩堂可一拼，但調堂後同一班當日同一科不能 4 堂或以上　5）普通話／戲劇由另一位老師合班，不計節數　6）課室同一時間只能一班　7）可複合多名老師　8）重摘課不能調堂
          </CardDescription>
        </CardHeader>
          ...
          <div>
            <p className="mb-2 font-medium">
              {coverSuggestions.some((c) => c.combine)
                ? "合班／代堂建議（普通話／戲劇合班不計節數；可即時揀再轉入紀錄）"
